...
const Header = () => {
  ...
  const handleQuery = (e: React.FormEvent<HTMLFormElement>): void => {
    e?.preventDefault();
    navigate(
      `/pages/Search?query=${encodeURIComponent(query)}&number=${number}`
    );
  };
      ...
      <div className="grid grid-rows-2">
        <div className="flex justify-between items-start sm:h-1/2 p-4">
          ...
          <div className="flex items-start">
            {!accessToken ? (
              <button
                className="rounded-2xl bg-milky-white px-4 py-1 mt-3 text-milky-brown items-center
        ...
        <div className="flex flex-col items-center justify-center px-4 sm:mt-8">
          <div>
            <h1
              className="text-white text-2xl font-montserrat my-4
          sm:mb-6 sm:text-3xl md:text-5xl"
            >
              {headline}
            </h1>
          </div>
