...
const Login = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (userData) {
            authFetch('accounts/auth/login/', userData);
        }
        nav('/');
    };
    ...
    return ( 
        <div className="flex justify-center items-center transition-all duration-500 ease-in-out my-12">    
            <form onSubmit={handleSubmit}>
                <h2 className="text-5xl mb-8 text-center"><b>Login</b></h2>
                ...
                <div className="bg-light-brown p-12 rounded-md text-xl">
                    ...
                    <div className="flex flex-wrap justify-between">
                        <button type="submit" className="bg-dark-brown py-2 px-4 rounded-full text-white hover:-translate-y-1 ease-in-out" >Sign in</button>
                        <button onClick={() => handleRedirect()} className="py-2 px-4 rounded-full text-light-brown bg-white hover:-translate-y-1 ease-in-out">Register</button>
                    </div>
                </div>
            </form>
        </div>
